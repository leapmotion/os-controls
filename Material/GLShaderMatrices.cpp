...
void GLShaderMatrices::UploadUniforms (ShaderBindRequirement req) const
{
  ...
  Matrix4x4f projection_matrix(m_projection.cast<float>());
  ...
  m_attached_shader->SetUniformMatrixf<4,4>("projection", projection_matrix, COLUMN_MAJOR);
  m_attached_shader->SetUniformMatrixf<4,4>("modelView", model_view_matrix, COLUMN_MAJOR);
  m_attached_shader->SetUniformMatrixf<4,4>("normalMatrix", normal_matrix, COLUMN_MAJOR);
