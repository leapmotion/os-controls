...
private:
  ...
  const EigenTypes::Vector3 VOLUME_SLIDER_OFFSET = EigenTypes::Vector3( 0.0f, 200.0f, 0.0f );
  ...
  void doMenuUpdate(const EigenTypes::Vector2& locationData, EigenTypes::Vector2 menuOffset);
  void doVolumeUpdate(const EigenTypes::Vector2& locationData, const EigenTypes::Vector2& deltaPixels, EigenTypes::Vector2 menuOffset);
  void resolveSelection(int selectedID);
  //Adjust the view for the volume control
  float calculateVolumeDelta(float deltaHandRoll);
  EigenTypes::Vector3 calculateBufferZoneOffset(const EigenTypes::Vector2& screenPosition);
