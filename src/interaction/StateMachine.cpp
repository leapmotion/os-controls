...
void StateMachine::AutoFilter(std::shared_ptr<Leap::Hand> pHand, const HandData& handData, const FrameTime& frameTime, const Scroll& scroll, ShortcutsState& state) {
...
void StateMachine::performNextTransition() {
  ...
  if (desiredState == ShortcutsState::FINAL) {
    if ( m_state == ShortcutsState::EXPOSE_FOCUSED ) {
      m_evp->Shutdown();
    }
