...
void PassthroughLayer::SetColorImage(const unsigned char* data) {
  m_colorimage.TexSubImage(Texture2PixelData(GL_RGBA, GL_UNSIGNED_BYTE, data, m_colorimage.Params().Width()*m_colorimage.Params().Height()*4));
  m_UseRGBI = true;
  m_HasData = true;
}

void PassthroughLayer::SetDistortion(const float* data) {
  m_distortion.TexSubImage(Texture2PixelData(GL_RG, GL_FLOAT, data, m_distortion.Params().Width()*m_distortion.Params().Height()*8));
}
...
void PassthroughLayer::Render(TimeDelta real_time_delta) const {
  if (m_HasData) {
    ...
    m_Shader->Bind();
    m_ShaderMatrices->SetMatrices(EigenTypes::Matrix4x4::Identity(), m_Renderer.Camera().ProjectionMatrix());
