...
void VolumeKnob::AnimationUpdate(const RenderFrame& renderFrame) {
  m_opacity.Update(renderFrame.deltaT.count());
...
void VolumeKnob::Render(const RenderFrame& renderFrame) const {
  m_knobBody.Draw(renderFrame.renderState);
  m_knobIndicator.Draw(renderFrame.renderState);
}
