...
void ExposeView::updateLayout(std::chrono::duration<double> dt) {
  ...
  const EigenTypes::Vector2 fullSize(fullScreen->Size().width, fullScreen->Size().height);
  const OSRect fullBounds = fullScreen->Bounds();
  const EigenTypes::Vector2 fullOrigin(fullBounds.origin.x, fullBounds.origin.y);
  const EigenTypes::Vector2 fullCenter = fullOrigin + 0.5*fullSize;

  auto screen = fullScreen->PrimaryScreen();
  const EigenTypes::Vector2 size(screen.Size().width, screen.Size().height);
  const OSRect bounds = screen.Bounds();
  const EigenTypes::Vector2 origin(bounds.origin.x, bounds.origin.y);
  m_viewCenter = origin + 0.5*size;

  // update background rectangle
  const EigenTypes::Vector2& bgSize = m_backgroundImage->Size();
  const double bgAspect = bgSize.x() / bgSize.y();
#if _WIN32
  const EigenTypes::Vector2 screenSize = fullSize;
  const EigenTypes::Vector2 screenCenter = fullCenter;
#else
  const EigenTypes::Vector2 screenSize = size;
  const EigenTypes::Vector2 screenCenter = m_viewCenter;
#endif
  ...
  m_backgroundImage->LinearTransformation() = bgScale*EigenTypes::Matrix3x3::Identity();

  // calculate radius of layout
  m_layoutRadius = 0.4 * std::min(size.x(), size.y());
  m_selectionRadius = 0.5 * m_layoutRadius;

  const EigenTypes::Vector2 screenToFullScale = size.cwiseQuotient(fullSize);
  ...
  for (const std::shared_ptr<ExposeViewWindow>& window : m_windows) {
    ...
    EigenTypes::Vector3 totalForce(EigenTypes::Vector3::Zero());
    ...
    const EigenTypes::Vector3 newPosition = window->m_position.Current() + m_alphaMask.Current()*window->m_grabDelta.Value() + m_alphaMask.Current()*window->m_forceDelta.Value();
    const EigenTypes::Vector3 delta = newPosition - window->m_prevPosition;
    const EigenTypes::Vector3 vel = delta / dt.count();
    window->m_velocity.SetGoal(vel);
    window->m_velocity.Update(static_cast<float>(dt.count()));
    window->m_prevPosition = newPosition;
    img->LinearTransformation() = (actualScale * EigenTypes::Matrix3x3::Identity()) * PrimitiveBase::SquashStretchTransform(0.03*window->m_velocity.Value(), EigenTypes::Vector3::UnitZ());
  ...
  for (const std::shared_ptr<ExposeGroup>& group : m_groups) {
    ...
    if (group->m_icon) {
      ...
      group->m_icon->LinearTransformation() = (0.25 + (1.0 - m_alphaMask.Current())) * EigenTypes::Matrix3x3::Identity();
...
void ExposeView::updateActivations(std::chrono::duration<double> dt) {
  ...
  POINT p;
  GetCursorPos(&p);
  const EigenTypes::Vector2 handPos(p.x, p.y);
  const float activation = (GetAsyncKeyState(VK_LBUTTON) & 0x8000) ? 1.0f : 0.0f;
#else
  const EigenTypes::Vector2& handPos = m_handData.locationData.screenPosition();
...
#endif
  static EigenTypes::Vector2 prevHandPos = handPos;

  EigenTypes::Vector3 displacement = EigenTypes::Vector3::Zero();
  const EigenTypes::Vector2 screenDeltaPos = m_handData.locationData.screenPosition() - m_prevHandData.locationData.screenPosition();
  ...
  for (const std::shared_ptr<ExposeViewWindow>& window : m_windows) {
    ...
    const EigenTypes::Vector2 windowPos = img->Translation().head<2>();
    const float scale = window->m_scale.Value();

    const EigenTypes::Vector2 windowSize = scale * img->Size();

    const EigenTypes::Vector2 min = windowPos - 0.5*windowSize;
    const EigenTypes::Vector2 max = windowPos + 0.5*windowSize;

    const EigenTypes::Vector2 closestPoint = handPos.cwiseMin(max).cwiseMax(min);
  ...
  for (const std::shared_ptr<ExposeViewWindow>& window : m_windows) {
    ...
    if (!m_ignoreInteraction && window == closestWindow && closestDistSq < distSqThreshPixels) {
      ...
      if ((img->Translation() - EigenTypes::Vector3(m_viewCenter.x(), m_viewCenter.y(), 0.0)).squaredNorm() < m_selectionRadius*m_selectionRadius) {
      ...
      window->m_grabDelta.SetGoal(EigenTypes::Vector3::Zero());
      window->m_selection.SetGoal(0.0f);
    }
...
EigenTypes::Vector2 ExposeView::radialCoordsToPoint(double angle, double distance) {
  return EigenTypes::Vector2(distance * std::cos(angle), distance * std::sin(angle));
}
...
void ExposeView::computeLayout() {
  ...
  const EigenTypes::Vector2 primarySize(primaryScreen.Size().width, primaryScreen.Size().height);
  const OSRect primaryBounds = primaryScreen.Bounds();
  const EigenTypes::Vector2 primaryOrigin(primaryBounds.origin.x, primaryBounds.origin.y);
  const EigenTypes::Vector2 primaryCenter = primaryOrigin + 0.5*primarySize;

  const EigenTypes::Vector2 primaryToFullScale = primarySize.cwiseQuotient(fullSize);

  const EigenTypes::Vector2 aspectScale(primarySize.x() / primarySize.y(), 1.0);
  ...
  for (const std::shared_ptr<ExposeGroup>& group : m_groups) {
    ...
    const EigenTypes::Vector2 cartesian = radialCoordsToPoint(groupAngle, groupRadius).cwiseProduct(aspectScale) + primaryCenter;
  ...
  for (const std::shared_ptr<ExposeGroup>& group : m_groups) {
    ...
    const EigenTypes::Vector2 scaledCenter = (group->m_center - primaryCenter).cwiseProduct(primaryToFullScale) + primaryCenter;
    ...
    for (const std::shared_ptr<ExposeViewWindow>& window : group->m_groupMembers) {
      ...
      angle += 0.5*curAngle;
      const EigenTypes::Vector2 cartesian = radialCoordsToPoint(angle, radius).cwiseProduct(aspectScale) + scaledCenter;
      const EigenTypes::Vector3 point3D(cartesian.x(), cartesian.y(), 0.0);
