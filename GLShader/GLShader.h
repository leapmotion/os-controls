...
public:
  ...
  void Bind () const {
    GL_THROW_UPON_ERROR(glUseProgram(m_prog));
  }
  ...
  static GLint CurrentlyBoundProgram () {
  ...
  void SetUniformi (const std::string &name, GLint value) {
    // TODO: type checking
    assert(CurrentlyBoundProgram() == m_prog && "trying to set a uniform without having first called GLShader::Bind on this");
    GL_THROW_UPON_ERROR(
  ...
  void SetUniformMatrixf (const std::string &name, const T_ &matrix, MatrixStorageConvention matrix_storage_convention) {
    ...
    assert(CurrentlyBoundProgram() == m_prog && "trying to set a uniform without having first called GLShader::Bind on this");
    GL_THROW_UPON_ERROR((
      glUniformMatrix4fv(glGetUniformLocation(m_prog, name.c_str()), 1, matrix_storage_convention == ROW_MAJOR, reinterpret_cast<const GLfloat *>(&matrix))
    ));
  }
  // Sets the named uniform to the given std::vector of values each of which must be
