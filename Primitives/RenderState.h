...
public:
  ...
  // upload functions
  void UploadMatrices();
  void UploadMaterial(const Color& diffuseColor, float ambientFactor, bool useTexture = false, GLint textureUnitIndex = 0);
  ...
  bool HavePositionAttribute() const { return m_shader->LocationOfAttribute("position") >= 0; }
  bool HaveNormalAttribute() const { return m_shader->LocationOfAttribute("normal") >= 0; }
  bool HaveColorAttribute() const { return m_shader->LocationOfAttribute("color") >= 0; }
  bool HaveTexCoordAttribute() const { return m_shader->LocationOfAttribute("texCoord") >= 0; }
